...
class RuleFit(BaseEstimator, TransformerMixin):
    ...
    def __init__(
            ...
            max_iter=None,
            n_jobs=None,
            random_state=None, fit_lasso=True):
    ...
    def fit(self, X, y=None, feature_names=None, restart=True):
        ...
        if self.tree_generator is None:
            ...
            if self.rfmode=='regress':
                self.tree_generator = GradientBoostingRegressor(
                    n_estimators=n_estimators_default, max_leaf_nodes=self.tree_size,
                    learning_rate=self.memory_par,subsample=self.sample_fract_,
        ...
        self._extract_rules(tree_list, X, y)
        return self
    ...
    def bag_trees(self, X, y):
        ...
        self._extract_rules(bagged_tree_list, X, y)
        return self

    def _extract_rules(self, tree_list, X, y):
        
        if isinstance(
            self.tree_generator, RandomForestRegressor) or isinstance(
    ...
    def get_rules(self, exclude_zero_coef=False, subregion=None):
        ...
        if self.fit_lasso:
            for i in range(0, len(self.rule_ensemble.rules)):
                ...
                output_rules += [(rule.__str__(), coef,  rule.support, importance)]
            rules = pd.DataFrame(output_rules, columns=["rule", "coef", "support", "importance"])
            ...
            for i in range(0, len(self.rule_ensemble.rules)):
                rule = rule_ensemble[i]
                output_rules += [(rule.__str__())]
            rules = pd.DataFrame(output_rules, columns=["rule"])
        return rules